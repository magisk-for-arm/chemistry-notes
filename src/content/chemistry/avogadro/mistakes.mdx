...
$\ce{NaCl}$ 是**离子化合物**，由 $\ce{Na+}$ 和 $\ce{Cl-}$ 通过离子键构成，**不存在 NaCl 分子**。1 mol $\ce{NaCl}$ 完全电离出 1 mol $\ce{Na+}$ 和 1 mol $\ce{Cl-}$，共 $2N_A$ 个离子。
...
- 1 mol $\ce{Na2O2}$ → $3N_A$ 个离子（2 mol $\ce{Na+}$ + 1 mol $\ce{O2^2-}$），注意阴离子是 $\ce{O2^2-}$ 而不是 $\ce{O^2-}$。
