...
category: "mistakes"
order: 1
summary: "13 条高频易错点，每条都配「错因剖析」与「接题方法」。"
...
下面 13 条按高频程度排列。建议先通读，再用文末的《接题方法总表》自查。
...
## 11. 混淆阿伏加德罗常数与近似值 6.02×10²³

<MistakeCard index={11} title="把阿伏加德罗常数与近似值 6.02×10²³ 混为一谈" frequency="medium" tags={['概念辨析']}>
<Callout type="wrong" title="错误示例">
...
<MistakeCard index={13} title="把离子化合物的离子数当成分子数" frequency="high" tags={['离子化合物', '离子数']}>
<Callout type="wrong" title="错误示例">

认为 1 mol $\ce{NaCl}$ 含 $N_A$ 个「$\ce{NaCl}$ 分子」，或说 1 mol $\ce{NaCl}$ 含 $N_A$ 个离子。

</Callout>

<Callout type="why" title="错因剖析">

$\ce{NaCl}$ 是**离子化合物**，由 $\ce{Na+}$ 和 $\ce{Cl-}$ 通过离子键构成，**不存在 NaCl 分子**。1 mol $\ce{NaCl}$ 中含 1 mol $\ce{Na+}$ 和 1 mol $\ce{Cl-}$，共 $2N_A$ 个离子。

</Callout>

<Callout type="right" title="正确做法">

- 1 mol $\ce{NaCl}$ → $2N_A$ 个离子（1 mol $\ce{Na+}$ + 1 mol $\ce{Cl-}$）；
- 1 mol $\ce{CaCl2}$ → $3N_A$ 个离子（1 mol $\ce{Ca^2+}$ + 2 mol $\ce{Cl-}$）；
- 1 mol $\ce{Na2O2}$（按化学式组成计）→ $3N_A$ 个离子（2 mol $\ce{Na+}$ + 1 mol $\ce{O2^2-}$），注意阴离子是 $\ce{O2^2-}$ 而不是 $\ce{O^2-}$。

</Callout>

<Callout type="skill" title="接题方法">

**「离子化合物先拆离子再相加」**：写出该物质的组成离子与个数，分别乘 $n$ 后求和。碰到「$\ce{NaCl}$ 分子」这种说法直接判错。
...
| 离子化合物（$\ce{NaCl}$、$\ce{CaCl2}$、$\ce{Na2O2}$） | 按组成离子计数，不存在「分子」 |
| 计算结果 | 单位、量级、有效数字是否正确？ |
